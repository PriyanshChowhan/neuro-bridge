...
export default function Signup() {
  ...
  const onSubmit = async (e) => {
    ...
    setBusy(true);
    try {
      const res = await api.post("/user", { username, emailId, password });
      const id = res?.data?.user?._id || res?.data?.data?._id || res?.data?._id;
      if (!id) throw new Error("Account created, but user id was missing.");
